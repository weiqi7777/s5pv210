...
#define REG_EPLL_LOCK	(ELFIN_CLOCK_POWER_BASE + EPLL_LOCK_OFFSET)
...
#define REG_EPLL_CON0	(ELFIN_CLOCK_POWER_BASE + EPLL_CON0_OFFSET)
...
#define rREG_EPLL_LOCK	(*(volatile unsigned long *)REG_EPLL_LOCK)
...
#define rREG_EPLL_CON0	(*(volatile unsigned long *)REG_EPLL_CON0)
...
void clock_init(void)
{
	...
	*/
	...
	set PLL
		...
		EPLL_FOUT = (EPLL_MDIV + K/65536)*FIN/(EPLL_PDIV*2^EPLL_SDIV) = 48 * 24 / (3 * 2^2) 96M
	*/
	rREG_APLL_CON0 = APLL_VAL;
	rREG_MPLL_CON  = MPLL_VAL;
	rREG_EPLL_CON0 = EPLL_VAL;
